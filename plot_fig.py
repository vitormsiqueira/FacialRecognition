...
face2_igual = np.load('arrays_saida/data_face2_igual_202001271800.npy') # load
face2_outro = np.load('arrays_saida/data_face2_outro_202001271800.npy') # load
...
def plot_roc_curve(igual, outro, cor):
    ...
    a, b, c = metrics.roc_curve(array_f, array, pos_label=2)
    faceAuc = auc(b, a)
...
def plot_histogram(face_igual, face_outro, vgg_igual, vgg_outro, cor1, cor2):
    ...
    # plt.subplot(1, 2, 2)
    plt.hist(vgg_igual, 50, alpha = 0.8, facecolor=cor1)
    plt.hist(vgg_outro, 50, alpha = 0.8, facecolor=cor2)
...
plot_histogram(face_igual, face_outro, face2_igual, face2_outro, verde, azul)
